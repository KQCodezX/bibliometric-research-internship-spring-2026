...
agg <- data.frame(
  year   = c(2015, 2015, 2016, 2016, 2017, 2017, 2018, 2018, 2020, 2021, 2022, 2022, 2023, 2023),
  status = c(
    ...
    "Not Reported",                        
    "Not Reported", "Not Reported",          
    "Not Reported", "Not Applicable (Protocol)"
  ),
...
counts$status <- factor(counts$status,
  levels = c("Not Reported", "Not Applicable (Protocol)"))
...
fig6c <- ggplot(counts, aes(x = year_label, y = count, fill = status)) +
  ...
  scale_fill_manual(
    values = c(
      "Not Reported"              = "#90A4AE",
      "Not Applicable (Protocol)" = "#CFD8DC"
    ),
    name = "Race/Ethnicity\nReporting Status"
  ) +
  ...
  labs(
    title    = "Reporting of Participant Race/Ethnicity in BCI-Spinal Cord Injury Studies",
    subtitle = "Figure 6c \u2014 Filtered articles (n=17) by publication year",
    x        = "Publication Year",
    y        = "Number of Studies",
    caption  = paste0(
      "Note: None of the 17 filtered studies explicitly reported participant race or ethnicity.\n",
      "Demographic tables included sex, age, injury level, and clinical measures only.\n",
      "'Not Applicable (Protocol)' = Colamarino 2023, a study protocol with no enrolled participants yet.\n",
      "This figure highlights a gap in demographic reporting transparency in BCI-SCI research."
    )
  ) +
